...
int height(struct tree *root)
{
	...
	else
	{
		...
		if(val1<val2)return val2+1;
		else return val1+1;
	}
}
void level_order(struct tree *root,int val)
{
	if(root==NULL)return ;
	else if(val==1)printf("%d ",root->info);
	else if(val>1)
	{
		level_order(root->left,val-1);
		level_order(root->right,val-1);
	}
}
void level(struct tree *root,int h)
{
	int i;
	for(i=1;i<=h;i++)
		level_order(root,i);
